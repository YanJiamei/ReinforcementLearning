...
class Learner:
    ...
    def reload_buffer(self, action, buff_data, action_base=1.0):
        if action>0 :
            self.action_flag=0
            ...
            self.buffer_data = buff_data
            self.buffer_data['weights']=1.0
        else:
            self.action_flag=1

    def step(self, trans_penalty=1.5, action_base = 1.0):
        ...
        reward = entro_gap * self.action_flag - trans_penalty * self.action_flag - self.H_ii_t
